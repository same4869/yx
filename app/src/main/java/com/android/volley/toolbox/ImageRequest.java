...
public class ImageRequest extends Request<Bitmap> {
	 ...
	 * natural size. If one of the two is nonzero, that dimension will be clamped and the other one will be set to preserve the image's aspect ratio. If both
	 * width and height are nonzero, the image will be decoded to be fit in the rectangle of dimensions width x height while keeping its aspect ratio.
	 * 
	 * @param url
	 *            URL of the image
	 * @param listener
	...
	private Response<Bitmap> doParse(NetworkResponse response) {
		...
		if (bitmap == null) {
			...
			//对rgb进行判断，如果指定config < bitmap.config，对rgb进行转换
			Config config = bitmap.getConfig();
			if(config != null && mDecodeConfig != null && mDecodeConfig.ordinal() < config.ordinal()){
