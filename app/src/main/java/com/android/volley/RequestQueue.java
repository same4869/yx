 ...
 * <p/>
 * Calling {@link #add(Request)} will enqueue the given Request for dispatch, resolving from either cache or network on a worker thread, and then delivering a
 * parsed response on the main thread.
 */
...
public class RequestQueue {
     ...
     * <li>get(cacheKey) returns waiting requests for the given cache key. The in flight request is <em>not</em> contained in that list. Is null if no requests
     * are staged.</li>
     * </ul>
     */
    ...
    /**
     * The set of all requests currently being processed by this RequestQueue. A Request will be in this set if it is waiting in any queue or currently being
     * processed by any dispatcher.
     */
