...
public class WaterWaveView extends View implements Runnable {
    ...
    private int firstColor = 0x9e736B11;
    private int secondColor = 0x5e736B11;
    ...
    private Path getSecondWavePath() {
        ...
        for (float x = 0; x <= width; x += X_STEP) {
            float y = (float) (waveHeight * Math.cos(omega * x + moveWave * 1.7f) + waveHeight) + heightOffset;
    ...
    private Path getFristWavePath() {
        ...
        for (float x = 0; x <= width; x += X_STEP) {
            float y = (float) (waveHeight * Math.sin(omega * x + moveWave) + waveHeight) + heightOffset;
    ...
    public void setHeightOffsetByProgress(int progress) {
        ...
        if (progress >= 0 && progress <= 100) {
            heightOffset = (100 - progress) * height * 0.01f;
        }
    }
